...
- **The Bicep template compiles but has never been deployed.** `az bicep build` passes in CI, so
  the template is syntactically valid and its resource types and API versions resolve. Nothing has
  been provisioned from it, so the resource configuration itself is unverified — role assignments,
  Easy Auth and identity-based storage access all need a real DEV deployment to confirm.
